...
module Logger {
    var logHistory:Array<any> = [],
        ...
        isDebug:boolean = $body.attr('data-debug') ? ($body.attr('data-debug') === 'true' ? true : false) : false,
        showLineNumbers:boolean = $body.attr('data-debug-lines') ? ($body.attr('data-debug-lines') === 'true' ? true : false) : false,
        errorCount:number = 0;
    ...
    export function warn(...args:any[]):void{
        if (window.console && window.console.warn && isDebug) {
            window.console.warn(Array.prototype.slice.call(args));
        }
    ...
    export module Analytics {
        var analyticsHistory:Array<any> = [],
            TRACKABLES:any = {},
            IS_ENABLED:boolean = true,//($(document.body).attr('data-prod') === '1' ? true : false) || false,
            _init:boolean = false;
        ...
        export function bindClickFragments():void {
            if(!IS_ENABLED){ log('Analytics !IS_ENABLED'); return; }
    ...
    function enableLineNumbers(yesno:boolean):void {
        if(yesno === true){
            // enable log caller monitoring
            if(window && window.console){
                if(!window['debug_log'].hasOwnProperty('ogLog')){
                    window['debug_log']['ogLog'] = window.console.log;
                }
                var ogLog = window.console.log;
                window.console.log = function() {
                  var err:any = new Error();

                  if(err && err.stack){
                      var args = [].slice.apply(arguments).concat(['\n\t'+err.stack.split(/\n/)[3].trim()]);
                      return ogLog.apply(this, args);
                  }else{
                      return ogLog(arguments);
                  }
                };
            }
        }else if(window['debug_log'].hasOwnProperty('ogLog')){
            window.console.log = window['debug_log']['ogLog'];
        }
    }
    ...
    window['debug_log'] = window['debug_log'] || {
        ...
        'force': function(yesno:boolean, withLines:boolean = false){
            isDebug = yesno;
            if(withLines === true && yesno === true){
                enableLineNumbers(withLines);
            }else if(yesno === false || withLines === false){
